...
def makePOSsentences(conllufilepath):
    ...
    for line in fh:
        if line == "\n":
            ...
            pos_tag = line.split("\t")[3] # UPOS token
...
def getScoringFeatures(dirpath,lang,err):
    ...
    for filename in files:
        ...
        if filename.endswith(".txt") and filename != ".parsed.txt":
            ...
            print(f"Got getLexFeatures for ONE file in {time.time() - pt} sec.")
    print(f"Got getScoringFeatures for ALL file in dir {dirpath} in {time.time() - pt0} sec.")

    print("Out getScoringFeatures")
...
def train_onelang_classification(train_labels,train_data,labelascat=False, langslist=None):
    uni_to_tri_vectorizer =  CountVectorizer(analyzer = "word", tokenizer = None, preprocessor = None, stop_words = None, ngram_range=(1,5), min_df=10) # Extrait TOUS les ngrams
    ...
    for vectorizer in vectorizers:
        for classifier in classifiers:
            ...
            print(cross_val)
            print(sum(cross_val)/float(len(cross_val)), f1_score(train_labels,predicted,average='weighted')) ##### LE WEIGHTED EST LÀ
...
def do_mega_multilingual_model_all_features(lang1path,lang1,lang2path,lang2,lang3path,lang3,modelas, setting,labelascat):
   ...
   if not setting == "domain":
      ...
      # lang1files = ["my_file_C2.txt.parsed.txt", ...]
      # lang1feautures = [doc1, doc2, ...] # ex: "POS POS POS\nPOS ..."
...
def do_cross_lang_all_features(sourcelangdirpath,sourcelang,modelas, targetlangdirpath, targetlang):
   ...
   if "cz" not in [sourcelang, targetlang]:
      ...
      #   mean_imputer = Imputer(missing_values='NaN', strategy='mean', axis=0) # imputer = SimpleImputer(missing_values=np.nan, strategy='mean')
...
def do_single_lang_all_features(langdirpath,lang,modelas):
   ...
   #    print("Modified domain feature vector for Italian")
    ...
    if modelas == "class":
       ...
       # vs pour le mega model : train_onelang_classification(megalabels,megadata,labelascat,megalangs)
...
def main():
    ...
    # print()
    #
    # # for features in ["pos", "word", "dep", "domain"]:
